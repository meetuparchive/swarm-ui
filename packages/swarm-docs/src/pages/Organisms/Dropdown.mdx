import { Menu, MenuList, MenuButton, MenuItem, MenuLink } from '@meetup/swarm-components';
import { Edit } from '@meetup/swarm-icons/lib/components/line';
import PropsTable from '../../components/propstable';
import Example from '../../components/examples/dropdown';

# Dropdown
---
An accessible set of components used to compose our dropdown menus.

```jsx
import { 
  Menu,
  MenuList,
  MenuButton,
  MenuItem,
  MenuLink
  } from '@meetup/swarm-components';
```

<Example />

Manage Group Example
```.jsx
<Menu>
  <MenuButton icon="arrow-down" right>
    Manage Group
  </MenuButton>
  <MenuList focusableChildrenTypes={[MenuItem]}>
    <MenuItem onSelect={() => alert("Download")}>Contact Group Members</MenuItem>
    <MenuItem onSelect={() => alert("Copy")}><Edit style={{ marginRight: 10 }} />Edit Group Settings</MenuItem>
    <MenuItem onSelect={() => alert("Mark as Draft")}>Mark as Draft</MenuItem>
    <MenuLink
      as="a"
      href="https://meetup.com"
    >Meetup</MenuLink>
    <MenuItem onSelect={() => alert("Delete")}>Delete</MenuItem>
  </MenuList>
</Menu>
```

Create Event Example
```.jsx
<Menu>
  <MenuButton primary icon="arrow-down" right>
    Create Event
  </MenuButton>
  <MenuList focusableChildrenTypes={[MenuItem]}>
    <MenuItem onSelect={() => alert("Download")}>Create New Event</MenuItem>
    <MenuItem onSelect={() => alert("Copy")}>Book as Space</MenuItem>
    <MenuItem onSelect={() => alert("Mark as Draft")}><Edit style={{ marginRight: 10 }} />Mark as Draft</MenuItem>
    <MenuLink
      as="a"
      href="https://meetup.com"
    >Meetup</MenuLink>
    <MenuItem onSelect={() => alert("Delete")}>Delete</MenuItem>
  </MenuList>
</Menu>
```

## Props
Dropdowns are composed of five components, their props are listed below:
<br/>
<PropsTable propMetaData={Menu.__docgenInfo} />
<PropsTable propMetaData={MenuList.__docgenInfo} />
<PropsTable propMetaData={MenuButton.__docgenInfo} />
<PropsTable propMetaData={MenuItem.__docgenInfo} />
